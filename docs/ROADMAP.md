...
**Current state:** The platform fully supports OTHR (Table 2) token type with 257 existence + 224 value + 6 LEI + 1 duplicate = 488 active assertions, PDF/DOCX/ODT/RTF extraction, and iXBRL generation. Block tagging (ix:continuation/ix:exclude), duplicate fact detection, multi-language support (24 EU languages), and GLEIF LEI lookup are implemented. All processing is in-memory and synchronous.
...
### 3. Multi-Language Output — **DONE**

**Implemented:** The `language-support` module validates ISO 639-1 codes for all 24 EU official languages (BG, CS, DA, DE, EL, EN, ES, ET, FI, FR, GA, HR, HU, IT, LT, LV, MT, NL, PL, PT, RO, SK, SL, SV). The English framework is extensible for future language packs with localized taxonomy labels.
...
### 4. Block Tagging (`ix:continuation` / `ix:exclude`) — **DONE**

**Implemented:** `ix:continuation` and `ix:exclude` are implemented in `inline-tagger.ts` and `section-renderer.ts`. Multi-fragment block tagging works for `textBlockItemType` elements, and non-reportable content (headers, footers, page numbers) is excluded from tagged blocks.

### 5. Duplicate Fact Detection — **DONE**

**Implemented:** The `duplicate-detector` module detects duplicate facts by concept+context+unit key and is integrated into the validation orchestrator. Inconsistent numeric duplicates produce ERRORs, inconsistent non-numeric duplicates produce WARNINGs, and consistent duplicates are allowed.
...
### 4. GLEIF LEI Lookup — **DONE**

**Implemented:** The `gleif-lookup` module validates LEI against the GLEIF API with a 5-second timeout. An `/api/lei-lookup` route is available. Graceful fallback to format/checksum validation when the API is unavailable.
